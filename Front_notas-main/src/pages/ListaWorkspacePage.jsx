...
export default function ListaWorkspacePage({
    ...
    try {
      ...
      const cuerpo = notaEditando
        ? {
            ...
            titulo,
            contenido,
            link_archivo: esPremium ? linkArchivo : '',
            accion: 'editar'
          }
        : {
            espacio_trab_id: workspaceActivo,
            id_usuario: usuario.id,
            titulo,
            contenido,
            link_archivo: esPremium ? linkArchivo : '',
      ...
      if (data.status === 'success') {
        ...
        if (refrescarWorkspaces) await refrescarWorkspaces();
        cerrarModal();
  ...
  const editarNota = (workspaceId, nota) => {
    ...
    setLinkArchivo(esPremium ? (nota.link_archivo || '') : '');
    ...
    <div className="space-y-5">
      {workspaces.length === 0 && (
        <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 py-16 px-8 text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700">Sin espacios de trabajo</h2>
        </div>
      )}

      {workspaces.map((workspace) => (
        <div key={workspace.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="bg-gray-50 p-5 flex justify-between items-center">
            <div
              onClick={() => setWorkspaceAbierto(workspaceAbierto === workspace.id ? null : workspace.id)}
              className="flex items-center gap-3 cursor-pointer"
            >
              <NotebookPen size={20} />
              <h2 className="text-xl font-bold text-gray-800">{workspace.nombre}</h2>
              {workspaceAbierto === workspace.id ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
            </div>
            <button
                    ...
                    <div className="space-y-2 w-full">
                      ...
                      {nota.link_archivo && (
                        <div className="mt-2 max-w-xs rounded-lg overflow-hidden border border-gray-200">
                          <img src={nota.link_archivo} alt="Archivo adjunto" className="max-h-40 object-cover" />
                        </div>
                      )}
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => editarNota(workspace.id, nota)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><Pencil size={16} /></button>
                      <button onClick={() => eliminarNota(nota.id)} className="p-2 text-rose-600 hover:bg-rose-50 rounded-lg"><Trash2 size={16} /></button>
                    </div>
                  </div>
                ))
              )}
      ...
      {workspaceActivo && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white w-[450px] rounded-2xl p-6 shadow-2xl">
            <h2 className="text-2xl font-bold mb-5">{notaEditando ? 'Editar Nota' : 'Nueva Nota'}</h2>
            <input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Título" className="w-full border rounded-xl p-3 mb-4" />
            <textarea value={contenido} onChange={(e) => setContenido(e.target.value)} placeholder="Contenido..." rows={4} className="w-full border rounded-xl p-3 mb-4" />
            {esPremium && (
              <input value={linkArchivo} onChange={(e) => setLinkArchivo(e.target.value)} placeholder="Enlace de imagen" className="w-full border rounded-xl p-3 mb-4" />
            )}
            <div className="flex gap-3 justify-end">
              <button onClick={cerrarModal} className="px-4 py-2 border rounded-xl">Cancelar</button>
              <button onClick={guardarNota} className="px-4 py-2 bg-rose-600 text-white rounded-xl">{notaEditando ? 'Actualizar' : 'Guardar'}</button>
            </div>
          </div>
        </div>
      )}
