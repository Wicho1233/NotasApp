...
export default function PrincipalPage({ logout }) {
  ...
  const refrescarWorkspaces = async () => {
    ...
    try {
      const response = await fetch(`https://notasapp-1.onrender.com/ver-espacios?id_usuario=${usuario.id}`);
      ...
      if (Array.isArray(listadoEspacios)) {
        const espaciosConNotas = await Promise.all(
          listadoEspacios.map(async (espacio) => {
            try {
              const resNotas = await fetch(`https://notasapp-1.onrender.com/ver-paginas?espacio_trab_id=${espacio.id}`);
